 ...
 * @returns {Promise}
 */
export function askAppChoice ({ dialogTitle, dialogMessage, cancelText, appsWhiteList, prefixes, appTitles, usingExpo, forceAppList }) {
  return new Promise(async (resolve) => {
    let availableApps = null;
    if(usingExpo){
      availableApps = forceAppList;
    }else{
      availableApps = await getAvailableApps(prefixes)
    }
